...
def predict(interpreter, input_details, output_details, input_array, input_size):
	""" 
	This function performs object detections on given image data with tensorflow lite 
	interpreter. After prediction it filers the result and applyes the NMS(non maximum suppression)
...
def process_predictions(predictions, image_id):
	...
	It returns the resultant dict after serializing it with 'json.dumps' method.
...
def get_class_names(classes):
	...
	classes name coresponding the class_id using 'read_class_names' function.
...
def is_blured(image, threshold=100):
	""" 
	This function checks if the given image is blur of not. If blur the return True otherwise False.
...
def capture_perfect(camera, MODEL_SIZE, max_capture, blur_threshold):
	...
	the image the it will return it otherwise it will try again at most max_capture(int) times.
...
def delete_imgs(im_dir, img_list):
	""" 
	This function deletes the image provided in img_list form the im_dir path.
...
def get_available_img(image_dir):
	""" 
	This function returns the number of image available at the image_dir
